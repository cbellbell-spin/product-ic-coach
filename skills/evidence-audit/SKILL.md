...
Uses the umbrella's Phase 1 → Phase 2 → Land mechanics, scoped to evidence quality.
...
For every distinct evidence claim, grade along five dimensions:
...
When `/land` is invoked: state (1) the strongest version of what the evidence does support, (2) the weakest claim that's load-bearing, (3) the cheapest next observation that would reduce the most uncertainty.
